...
public partial class Search_OP : System.Web.UI.Page
{
    ...
    protected void BtnSearch_Click(object sender, EventArgs e)
    {
        ...
        if (SelOP.SelectedValue != "" | YYYYMM != "______")
        {
            string SQL = "SELECT distinct [日誌班別],[日誌編號] FROM [View_工作日誌] WHERE not ([員工代號]<>'" + Request.Cookies["UserID"].Value.ToString() + "' and [處理過程] like '%@@%')";

            //---------------------------------------------------------人員
            if (SelOP.SelectedValue != "#") SQL = SQL + " and [當班OP] like '%" + SelOP.SelectedItem.Text + "%'";
            else SQL = SQL + " and [員工代號] not in (select [代號] from [View_組織架構] where [課別]='作業管理科' and [性質]='員工')";

            //---------------------------------------------------------排除
            if (ChkOut.Checked) SQL = SQL + " and [訊息代碼] not in (select [Config] from [Config] where [Kind]='排除次數')";
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        if (SelOP.SelectedValue != "" | YYYYMM != "______")
        {
            ...
            //---------------------------------------------------------人員
            if (SelOP.SelectedValue != "#") SQL = SQL + " and [員工代號]='" + SelOP.SelectedValue + "'";
            else SQL = SQL + " and [員工代號] not in (select [代號] from [View_組織架構] where [課別]='作業管理科' and [性質]='員工')";
