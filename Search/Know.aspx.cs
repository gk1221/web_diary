...
public partial class Search_Know : System.Web.UI.Page
{
    ...
    protected void Page_PreRenderComplete(object sender, EventArgs e)   //放Page_Load無法顯示
    {
        ...
        if (!IsPostBack)
        {
            ...
            else if (Request["SysCode"] != null)
            {
                string SysCode = Request["SysCode"].ToString();
                SelYYYY.SelectedIndex = 0;
                SelMM.SelectedIndex = 0;
                for (int i = 0; i < SelSys.Items.Count; i++) if (SelSys.Items[i].Value == SysCode) SelSys.Items[i].Selected = true;
    ...
    {
        //if (SelOP.SelectedValue != "")
        //{
            string SQL = "SELECT distinct [日誌班別],[日誌編號] FROM [View_工作日誌] WHERE not ([員工代號]<>'" + Request.Cookies["UserID"].Value.ToString() + "' and [處理過程] like '%@@%')";

            //---------------------------------------------------------知識(0：無 1：追蹤 2：結案 3：未定)
            SQL = SQL + " and ([狀態代碼]>0 and [公告期限] like 'yyyy%' or [狀態代碼]=3)";

            //---------------------------------------------------------系統
            if (SelSys.SelectedValue != "") SQL = SQL + " and ([訊息代碼] like '_" + SelSys.SelectedValue + "%' or [系統代碼]='" + SelSys.SelectedValue + "')";

            //---------------------------------------------------------日期
            if (SelYYYY.SelectedValue + SelMM.SelectedValue != "______") SQL = SQL + " and [處理時間] like '" + SelYYYY.SelectedValue + "/" + SelMM.SelectedValue + "%'";

            //---------------------------------------------------------關鍵字
            String[] keyA = txtKey.Text.Trim().Split(' ');
            foreach (string key in keyA) if (key.Trim() != "") SQL = SQL + " and [處理過程] like '%" + key + "%'";
