...
public partial class Search_EmsSearch : System.Web.UI.Page
{
    protected void Page_Preload(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            GridView1.Columns[1].HeaderStyle.Width = 50;
        }
    } 
    ...
    protected void BtnSearch_Click(object sender, EventArgs e)
    {
        ...
        if (YYYYMM != "____-__" | txtKey.Text != "")
        {
            string SQL = "SELECT * FROM [View_EmsLog] where 1=1";

            //---------------------------------------------------------表格
            if (SelTbl.SelectedValue != "") SQL = SQL + " and [tbl]='" + SelTbl.SelectedValue + "'";

            //---------------------------------------------------------關鍵字
            if (txtKey.Text.Trim() != "") SQL = SQL + " and [EmsLog] like '%" + txtKey.Text.Trim() + "%'";

            //---------------------------------------------------------日期
            if (YYYYMM != "____-__") SQL = SQL + " and [EmsDT] like '" + YYYYMM + "%'";
            ...
            GridView1.Sort("[EmsDT]", SortDirection.Ascending);
            GridView1.DataBind();
            //Response.Write(SQL);
            ViewState["SQL"] = SQL;
        }
