...
public partial class Search_ManuSearch : System.Web.UI.Page
{
    ...
    protected void BtnSearch_Click(object sender, EventArgs e)
    {
        ...
        string SQL = "SELECT * FROM [SopLog] where 1=1"; 

        //---------------------------------------------------------日期
        string MM = SelMM.SelectedValue; if (MM.Substring(0, 1) == "0") MM = MM.Substring(1, 1);
        if (YYYYMM != "____/__") SQL = SQL + " and ([SaveDT] like '" + YYYYMM + "%' or [SopCode] in (select [訊息代碼] from [View_SOP] where [更新日] like '" + SelYYYY.SelectedValue + "/" + MM + "%'))";

        //---------------------------------------------------------系統
        if (SelSys.SelectedValue != "") SQL = SQL + " and ([SopCode] like '_" + SelSys.SelectedValue + "%')";

        //---------------------------------------------------------僅停用
        if (ChkEnd.Checked) SQL = SQL + " and ([Status]='停用')";
        ...
        GridView1.Sort("[SaveDT]", SortDirection.Ascending);
        GridView1.DataBind();
        //Response.Write(SQL);
        ViewState["SQL"] = SQL;
    }
