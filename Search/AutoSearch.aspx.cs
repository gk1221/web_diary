...
public partial class Search_AutoSearch : System.Web.UI.Page
{
    ...
    protected void BtnSearch_Click(object sender, EventArgs e)
    {
        ...
        if (MsgCode != "_______" | YYYYMM != "____/__")
        {
            string SQL = "SELECT * FROM [WarnLog] where 1=1"; 

            //---------------------------------------------------------訊息
            if (MsgCode != "_______") SQL = SQL + " and [WarnCode] like '" + MsgCode + "'";                             
            
            //---------------------------------------------------------日期
            if (YYYYMM != "____/__") SQL = SQL + " and [WarnDT] like '" + YYYYMM + "%'"; 
            ...
            GridView1.Sort("[WarnDT]", SortDirection.Ascending);
            GridView1.DataBind();
            //Response.Write(SQL);
            ViewState["SQL"] = SQL;
        }
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)    //將SopDT存於SopCode.ToolTip
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            item.Cells[3].Text = Server.HtmlDecode(item.Cells[3].Text);
        }
    }
