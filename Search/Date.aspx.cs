...
public partial class Search_Date : System.Web.UI.Page
{
    ...
    {
        string strDate = SelYYYY.SelectedValue + "/" + SelMM.SelectedValue + "/" + SelDD.SelectedValue ;
        if (strDate != "____/__/__")
        {
            string SQL = "SELECT distinct [日誌班別],[日誌編號] FROM [View_工作日誌] WHERE not ([員工代號]<>'" + Request.Cookies["UserID"].Value.ToString() + "' and [處理過程] like '%@@%')";

            //---------------------------------------------------------日期
            if (SelItem.SelectedValue == "所有欄位")
            {
                SQL = SQL + " and ([日誌班別] like '" + SelYYYY.SelectedValue + SelMM.SelectedValue + SelDD.SelectedValue + "_'";
                SQL = SQL + " or   [公告期限] like '" + SelYYYY.SelectedValue + SelMM.SelectedValue + SelDD.SelectedValue + "_'";
                SQL = SQL + " or   [日誌時間] like '" + strDate + "%'";
                SQL = SQL + " or   [日誌存檔] like '" + strDate + "%'";
                SQL = SQL + " or   [訊息時間] like '" + strDate + "%'";
                SQL = SQL + " or   [訊息存檔] like '" + strDate + "%'";
                SQL = SQL + " or   [處理時間] like '" + strDate + "%'";
                SQL = SQL + " or   [處理存檔] like '" + strDate + "%'";
                SQL = SQL + " or   [叫修存檔] like '" + strDate + "%'";
                SQL = SQL + ")";
            }
            else if (SelItem.SelectedValue != "日誌班別" & SelItem.SelectedValue != "公告期限") SQL = SQL + " and [" + SelItem.SelectedValue + "] like '" + strDate + "%'";
            else SQL = SQL + " and [" + SelItem.SelectedValue + "] like '" + SelYYYY.SelectedValue + SelMM.SelectedValue + SelDD.SelectedValue + "%'";
