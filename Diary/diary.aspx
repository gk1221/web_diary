<%@ Page Title="當班日誌" Language="C#" MasterPageFile="../MasterPage.master"
...
>
  <div id="divBody" align="center" style="width: 100%; overflow: auto">
    <% MainDiary(); //當班日誌 MainSave(); //當班更改 MainTrace(); //追蹤公告 %>
  </div>
  <script type="text/javascript">
    ...
    function MsgDT_Click(qryTour, MsgNo, MsgCode, qryMsgDT) {
      var MsgDT = "";
      MsgDT = prompt(
        " 請輸入要修改的訊息時間，格式為：yyyy/mm/dd hh:mi",
        qryMsgDT
      ).toString();
      if (MsgDT != "")
        window.open(
    ...
    function Cmd_Click(tmpArea, DBaction) {
      //新增刪除修改
      if (qryArea != tmpArea)
        alert("請先點選您欲異動之記錄 !"); //選此qryArea卻點彼qryArea的Cmd
      else {
        var tmpText = "";
        ...
        switch (DBaction) {
          ...
          case "u": {
            ...
            //新增Msg
              ...
              if ((qryTbl == "Msg") & (DBaction == "u"))
                MsgDT_Click(qryTour, MsgNo, MsgCode, MsgDT); //修改Msg
              //新增或修改Process
              else {
                if (DBaction == "a")
                  window.open(
          ...
          case "d": {
            ...
            if (
              confirm(
                "<<注意>>！刪除該筆日誌到最後若只剩一筆發生訊息或處理過程之後，整筆日誌記錄，包含所有屬性設定，發生訊息與處理過程，均將一併刪除！建議以先新增再刪除的操作方式，可以避免誤刪資料。\n\n您確定刪除下列資料？\n\n " +
                  tmpText
              )
            ) {
              if (qryTbl == "Msg")
