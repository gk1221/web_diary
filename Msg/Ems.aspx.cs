...
public partial class Msg_Ems : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            lblEms.Text = "Ems更新：" + GetValue("Diary", "select max([DATA_TIME]) from [View_EmsAI]");
        }
    }      
    ...
    protected void Page_PreRenderComplete(object sender, EventArgs e)   //放Page_Load無法顯示
    {
        ...
        if (ViewState["SQL"] != null)
        {
            ...
            GridView1.Sort("[EmsDT]", SortDirection.Descending);
            GridView1.DataBind();
        }
        else if (Session["tour"].ToString() != "")    //
        {
            if (!Page.IsPostBack)
            {   
                SQL = "select *,'1992f06' as [MsgCode],substring([EmsDT],12,5) as [訊息時間] from [View_EmsLog] where [EmsDT] between " + GetTourSQL(Session["tour"].ToString()).Replace("/", "-");                
                ViewState["SQL"] = SQL;
