...
Function GetSQL(qryTour,which)
  ...
  if which="p" then
    ...
    case "3" : GetSQL="select * from Device2 where CreateDate between ' " & mid(qryTour,1,8) & " 19:30 " & "' and '" & mid(qryTour,1,8) & " 07:29 '"
    end select
  end if
  'WScript.Echo GetSQL
End Function
...
function GetDiaryNo(qryTour,MsgCode)
  ...
  'WScript.Echo "select * from Msg,Process where Msg.tour=Process.tour and Msg.DiaryNo=Process.DiaryNo and Msg.tour='" & qryTour & "' and MsgCode='" & MsgCode & "' and ProText like '人員進出：%'"
  rs.close
end function
'---------------刪除門禁資料---------------------------------------------------------
...
function InsControl(qryTour,MsgCode,MsgText,ProText)
  'WScript.Echo "insert into Process values('" & qryTour & "'," & DiaryNo & "," & ProcessNo & ",'" & nowDT & "','" & nowDT & "','" & ProText & "','SOS','operator','電腦操作課','機房OP','')"
  dim DiaryNo
